...
double red[10]   = {1, 0, 1, 0.9, 0.4, 0, 0, 0, 0, 0.5};
double green[10] = {1, 0, 1, 0, 0.1, 0.1, 0.5, 0.5, 0.5, 0.8};
double blue[10]  = {0, 1, 1, 0.5, 0.4, 0.5, 0.9, 0.5, 0.3, 0.1};
...
int main(int argc, char **argv)
{
  ...
  double sun[4][4];
  M3d_make_translation(sun, 0, 0, 6);
  M3d_mat_mult_points (x[0],y[0],z[0],  sun, x[0],y[0],z[0],numpoints[0]+1);


  double earthS[4][4], earthT[4][4];
  M3d_make_scaling(earthS, 0.4, 0.4, 0.4);
  M3d_make_translation(earthT, 3, 0, 6);
  M3d_mat_mult(earthT, earthT, earthS);
  M3d_mat_mult_points (x[1],y[1],z[1],  earthT, x[1],y[1],z[1],numpoints[1]+1);

  double moonS[4][4], moonT[4][4];
  M3d_make_scaling(moonS, 0.2, 0.2, 0.2);
  M3d_make_translation(moonT, 4, 0, 6);
  M3d_mat_mult(moonT, moonT, moonS);
  M3d_mat_mult_points (x[2],y[2],z[2],  moonT, x[2],y[2],z[2],numpoints[2]+1);
  ...
  double earthRotateSpeedSn = sin(0.98 * M_PI/180);
  double earthRotateSpeedCs = cos(0.98 * M_PI/180);

  double daySpeedSn = sin(15.2 * M_PI/180);
  double daySpeedCs = cos(15.2 * M_PI/180);

  double monthSpeedSn = sin(12 * M_PI/180);
  double monthSpeedCs = cos(12 * M_PI/180);
  ...
  double E[4][4];
  double yV[4][4], yZ[4][4], yT[4][4];
  double mV[4][4], mZ[4][4], mT[4][4];
  double dV[4][4], dZ[4][4], dT[4][4];
  double sV[4][4], sZ[4][4], sT[4][4];
  //G_rgb(1,0,0);
  //draw2();
  ...
  while (1) {
    ...
    M3d_make_identity(yV);
    M3d_make_identity(yZ);
    M3d_make_identity(yT);
    M3d_make_identity(mV);
    M3d_make_identity(mZ);
    M3d_make_identity(mT);
    M3d_make_identity(sV);
    M3d_make_identity(sZ);
    M3d_make_identity(sT);
    ...
    if (q == 'q') {
      ...
      //printf("%lf %lf %lf \n", x[1][rt], y[1][rt], z[1][rt]);

      M3d_make_translation(yZ, -x[0][rt], -y[0][rt], -z[0][rt]);
      M3d_make_y_rotation_cs(yV, earthRotateSpeedCs, earthRotateSpeedSn * sign);
      M3d_make_translation(yT, x[0][rt], y[0][rt], z[0][rt]);
      M3d_mat_mult(yV, yV, yZ);
      M3d_mat_mult(yV, yT, yV);

      
      
      M3d_make_translation(sZ, -x[0][rt], -y[0][rt], -z[0][rt]);
      M3d_make_y_rotation_cs(sV, earthRotateSpeedCs, earthRotateSpeedSn * - sign);
      M3d_make_translation(sT, x[0][rt], y[0][rt], z[0][rt]);
      M3d_mat_mult(sV, sV, sZ);
      M3d_mat_mult(sV, sT, sV);

      
      M3d_make_translation(mZ, -x[1][rt], -y[1][rt], -z[1][rt]);
      M3d_make_y_rotation_cs(mV, monthSpeedCs, monthSpeedSn * sign);
      M3d_make_translation(mT, x[1][rt], y[1][rt], z[1][rt]);
      M3d_mat_mult(mV, mV, mZ);
      M3d_mat_mult(mV, mT, mV);
    ...
    M3d_mat_mult_points (x[2],y[2],z[2],  mV, x[2],y[2],z[2],numpoints[2]+1);

    M3d_mat_mult_points (x[1],y[1],z[1],  yV, x[1],y[1],z[1],numpoints[1]+1);
      
    //M3d_mat_mult_points (x[1],y[1],z[1],  dV, x[1],y[1],z[1],numpoints[1]+1);
    
    M3d_mat_mult_points (x[0],y[0],z[0],  sV, x[0],y[0],z[0],numpoints[0]+1);
